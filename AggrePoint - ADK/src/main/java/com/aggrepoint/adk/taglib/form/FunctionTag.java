 ...
 * @author YJM
 */
public class FunctionTag extends BodyTagSupport implements IAdkConst,
		IWinletConst {
	...
	/** 用于在<adk:form>中自动生成设置form字段值的脚本：脚本参数对应的表单字段名 */
	Vector<String> m_vecParams;

	/** 用于在<adk:form>中自动生成设置form字段值的脚本：设置完毕是否提交表单 */
	 ...
	 * @return
	 */
	void genScript(FormTag form, String name, WinletReqInfo reqInfo,
			String winiid, String formName, String body) {
		...
		if (body == null && reqInfo.m_bUseAjax) {
			...
			form.setAdkformScript(name, sb.toString());
		} else {
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (m_strName != null) {
				...
				if (form != null) {
					// 先生成不包含BODY的版本
					// 以<adk:func name="xxx"/>方式使用时，doAfterBody()不会被调用
					// 因此在这里先生成一次
