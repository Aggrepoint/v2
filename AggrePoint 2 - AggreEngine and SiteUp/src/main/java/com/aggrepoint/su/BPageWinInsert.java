 ...
 * @author YJM
 */
public class BPageWinInsert extends BaseModule implements RuleConst,
		IParamConst {
	public static final int ACTION_SEL_WIN = 0;
	public static final int ACTION_PARAM = 1;
	public static final int ACTION_INSERT_WIN = 2;
	...
	public int commonProc(IModuleRequest req, IModuleResponse resp, int action)
		...
		long lContainer = req.getParameter(PARAM_CONTAINER_ID, 0l);
		...
		switch (action) {
		case ACTION_SEL_WIN: // 选择要插入的窗口
			app.m_lSiteID = page.m_lSiteID;
			req.setAttribute("APPS",
					adapter.retrieveMulti(app, "loadForInsert", null));
			break;
		case ACTION_PARAM:
		case ACTION_INSERT_WIN:
			win = new ApWindow();
			win.m_lWindowID = req.getParameter(PARAM_WIN_ID, -1l);
			...
			if (action == ACTION_PARAM) { // 指定插入参数
				...
				vecFrames.addAll(adapter.retrieveMulti(frame, "loadBySite",
						"order"));
				...
				cont.m_lWindowID = win.m_lWindowID;
				cont.m_strContName = win.m_strName;
				...
				if (vecFrames.size() > 0)
					cont.m_lFrameID = ((ApFrame) vecFrames.elementAt(0)).m_lFrameID;
	...
	public int execute(IModuleRequest req, IModuleResponse resp)
			throws Exception {
		return commonProc(req, resp, ACTION_SEL_WIN);
	}

	/**
	 * 指定插入的参数
	 */
	public int param(IModuleRequest req, IModuleResponse resp) throws Exception {
		return commonProc(req, resp, ACTION_PARAM);
	}
	...
	public int insert(IModuleRequest req, IModuleResponse resp)
			throws Exception {
		return commonProc(req, resp, ACTION_INSERT_WIN);
	}
}
